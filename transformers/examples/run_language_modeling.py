...
class SummQGTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            self.examples = []
            with open(file_path, encoding="utf-8") as f:
                lines = [line + "<|endoftext|>" for line in f.read().split('<|endoftext|>') if (len(line) > 0 and not line.isspace())]

            pad_token = tokenizer.pad_token
            if pad_token is None:
                print("Model does not have a pad token")
                print("Adding pad token to special tokens")
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if args.summ_qg_dataset:
        return SummQGTextDataset(tokenizer, args, file_path=file_path, block_size=args.block_size)
...
def main():
    ...
    parser.add_argument(
        "--summ_qg_dataset",
        action="store_true",
        help="Whether SummQG training sequences pre-processing is used, which makes use of right padding.",
    )
